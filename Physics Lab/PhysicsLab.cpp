...
bool PhysicsLab::init()
{
	...
	// reset joystick
	joyX = joyY = 0.0;
	minDistance = ConfigManager::getFloat("Plugin.PhysicsLab.MinDistance", MIN_DISTANCE);
	maxDistance = ConfigManager::getFloat("Plugin.PhysicsLab.MaxDistance", MAX_DISTANCE);

	// set up menu item and item callback
...
void PhysicsLab::addToScene(osg::MatrixTransform* node) {
	...
	if (camNode) camNode -> addChild(node); // add the new node as child of rootnode
}
...
void PhysicsLab::setupScene()
{
	...
	// Kai
	// y = max(min(y, maxDistance), minDistance);
	...
	// World 
	, Elevator, then Funnel
	of->addAntiGravityField(Vec3(0, 0, 0), Vec3(10000, 10000, 10000), Vec3(0, 0, -5000), true); // scene gravity
	camNode->addChild(of->addAntiGravityField(Vec3(-1850, 0, 700), Vec3(100, 100, 650), Vec3(0, 0, 500), false)); // funnel anti-gravity
	...
	{
		// Kai: Use an array to store osgb strings and parameter and use a loop to generate?
